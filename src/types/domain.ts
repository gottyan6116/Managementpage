...
/* ===== Issue Tree (論点ツリー / ロジックツリー / KPIツリー / 業務プロセスツリー) ===== */

export type IssueTreeKind = "issue" | "logic" | "kpi" | "process";
...
export interface IssueBoard {
  id: string;
  clientName: string;
  /** 既存の担当案件 (projects) との連携。タスク作成時の紐付け先 */
  projectId: string | null;
  name: string;
  category: string;
  objective: string;
